...
const About = () => {
  ...
  return (
    <section className=" px-6 md:px-16 pt-10 pb-24 bg-black/90 text-white grid lg:grid-cols-2 gap-14 items-center">
      {/* left */}
      <div className="relative max-w-md">
        <div className="absolute -inset-3 rounded-4xl border border-lime-300/10 blur-xl" />
        <HeroImageComponent />
        <div className="absolute -bottom-6 -right-6 bg-zinc-950/80 border border-white/10 rounded-3xl px-6 py-4 shadow-2xl">
          <p className="text-3xl font-light text-lime-300">3+ yrs</p>
      ...
      <div>
        ...
        <h2 className="text-5xl md:text-6xl font-serif leading-tight mb-6">
          About <span className="text-lime-300 italic">me</span>
        </h2>
        ...
        <div className="mt-10">
          ...
          <div className="flex flex-wrap gap-3">
            {strengths.map((item) => (
              <span
                key={item}
                className="px-4 py-2 rounded-full border border-lime-300/20 bg-lime-300/10 text-lime-200 text-sm"
              >
                {item}
              </span>
            ))}
